...
def mut_hist_somermeyer(data_dir: Path, save_path: Path = None):
    """
    Create grouped bar plot showing number of mutations for each GFP dataset.

    Parameters
    ----------
    data_dir : Path
        Directory containing full-named GFP CSV files.
    save_path : Path, optional
        If given, saves the figure to this path.
    """
    # Desired order of GFP datasets
    gfp_order = ["cgreGFP", "ppluGFP", "amacGFP"]
    ...
    for file_path in files:
        try:
            gene = convert_name_to_gfp(file_path.stem)
        except Exception:
            continue

        if gene not in gfp_order:
            continue
    ...
    # Count mutations
    df_all["num_mutations"] = df_all["mutant"].apply(
        lambda x: len(x.split(":")) if x != "wt" else 0
    )

    # Count number of variants > 10 per gene
    over_10 = (
        df_all[df_all["num_mutations"] > 10]
        .groupby("gene")
        .size()
        .reindex(gfp_order, fill_value=0)
        .to_dict()
    )
    ...
    for i, gene in enumerate(gfp_order):
        counts = grouped.loc[gene]
        ax.bar(
            x + i * width,
            counts.values,
            width=width,
            label=f"{gene}: {over_10.get(gene, 0)}"
        )

    ax.set_xlabel("Number of mutations")
    ax.set_ylabel("Frequency")
    ax.set_title("Distribution of the number of mutations")
    ax.set_xticks(x + width)
    ax.set_xticklabels(x)
    ax.legend(title="Variants > 10")
